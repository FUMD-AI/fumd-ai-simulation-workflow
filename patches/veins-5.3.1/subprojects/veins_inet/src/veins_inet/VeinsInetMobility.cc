...
// [FUMD-AI patch] Required for SUMO<->Veins mapping file output
...
void VeinsInetMobility::initialize(int stage)
{
    ...
    // [FUMD-AI patch] Emit SUMO vehicle ID as OMNeT++ signal for statistics recording
    // Reason: Allows the SUMO numeric ID to be captured in .sca/.vec output
    // alongside standard OMNeT++ statistics.
    // Reference: https://github.com/FUMD-AI/fumd-ai-simulation-workflow
...
void VeinsInetMobility::setInitialPosition()
{
    ...
    EV << "My SUMO id = " << sumoId
       << " - My VEINS id = " << veinsId << endl;  //this will be printed in the .out file (>tens of gigabytes)

    // Write to same directory as other simulation results
    std::string resultDir = getEnvir()->getConfig()->getConfigEntry("result-dir").getValue();
    if (resultDir.empty())
        resultDir = ".";
