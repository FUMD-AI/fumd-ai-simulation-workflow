...
void LteRlcTm::handleLowerMessage(cPacket *pkt)
{
    ...
    if (strcmp(pkt->getName(), "LteMacSduRequest") == 0) {
        ...
        // KG 2/7/2026
        // INET 4.x fix: cast to inet::Packet* before using tags API
        //auto *inetPkt = check_and_cast<inet::Packet *>(pkt);
        //auto lteInfoPtr = inetPkt->removeTag<FlowControlInfo>();
        //cPacket *upPkt = check_and_cast<cPacket *>(inetPkt->decapsulate());
        //cPacket *upUpPkt = check_and_cast<cPacket *>(upPkt->decapsulate());
        //upUpPkt->setControlInfo(lteInfoPtr->dup());  // dup() gives an ownable raw pointer
        //delete upPkt;
        // KG end
        ...
        // KG 3/7/2026
        auto inetPkt = check_and_cast<inet::Packet *>(pkt);
        EV << "LteRlcTm : Sending packet " << inetPkt->getName() << " to port TM_Sap_up$o\n";
